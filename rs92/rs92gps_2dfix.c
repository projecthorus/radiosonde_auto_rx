  ...
  //err2 |= get_GPSweek();
    ...
    if (!err2 && (almanac || ephem)) {
        ...
        if (k == 3) {
            ...
            fix2d = naiv_2Dfix( 3, Sat_A, fixalt2d);
        }
    }
    ...
    if (!err2) {
        ...
        if (k == 3  &&  fix2d > 0  &&  gpx.dop > 0  &&  gpx.dop < 2*dop_limit) {
            fprintf(stdout, "  2D  lat: %.4f  lon: %.4f  alt: %.1f ", gpx.lat, gpx.lon, gpx.alt);
...
int main(int argc, char *argv[]) {
    ...
    while ((*argv) && (!fileloaded)) {
        ...
        else if ( (strcmp(*argv, "--2dalt") == 0) ) {  // 2D-fix: --2dalt <alt>
            ++argv;                                    // alt: fixed altitude above ellipsoid
            if (*argv) {
                fixalt2d = atof(*argv);
                if (fixalt2d <= -1000  || fixalt2d >= 50000)  fixalt2d = 0.0;
            }
            else return -1;
        }
